"""ML logistic-regression executor that converts ML scores into backtest signals."""
...
class Strategy(StrategyBase):
    """Train logistic/elasticnet baselines then map probabilities to trading entries."""

    metadata = StrategyMetadata(
        name="ml_logreg",
        description=(
            "Pipeline ML multi-sumber yang melatih Logistic Regression berbasis L1/elasticnet "
            "lalu mengubah probabilitas menjadi sinyal long/short dengan guardrail leverage dan turnover."
        ),
        entry=(
            "Long ketika probabilitas model > 0.5 dan sinyal sebelumnya tidak long. Short ketika probabilitas < 0.5 "
            "dan sinyal sebelumnya tidak short. Probabilitas dikonversi menjadi sinyal [-1, 1] sesuai kontrak SignalBacktester."
        ),
        exit=(
            "Posisi ditutup saat sinyal berganti arah atau kembali ke netral (probabilitas = 0.5). Guardrail akan menskala "
            "bobot jika leverage melebihi batas atau turnover melampaui limit."
        ),
        parameters={
            "label_horizon": 24,
            "top_k": 1,
    ...
    def _build_configs(self) -> tuple[
        ...
        label_cfg = LabelConfig(horizon_bars=self.label_horizon, task="binary", threshold=0.0)
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        ml_probability = (ml_signal + 1.0) / 2.0
