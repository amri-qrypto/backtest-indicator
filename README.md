...
- **`notebooks/indikator-ml-strategy/indicator_ml_strategy.ipynb`** – Fokus pada data indikator Binance BTCUSDT 1H/4H/1D dengan LightGBM + SHAP. Bagian akhir memuat *Strategi referensi & cara uji (indikator + ML)* yang merangkum 10 strategi (reversal, trend-following, breakout, carry, dsb.), fitur wajib, aturan entry/exit, serta langkah praktis untuk menambahkan rule berbasis `prob_up`, tuning threshold, dan membandingkan lintas timeframe.
