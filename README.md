...
- **Exit & manajemen risiko** – Stop loss berbasis ATR (`atr_length=14`) dengan multiplier default 1.5. Posisi long keluar jika harga menyentuh `entry_price - ATR*multiplier`, posisi short keluar di `entry_price + ATR*multiplier`. Take profit opsional kini bisa diaktifkan via `atr_take_profit_multiplier` untuk menjaga rasio risk-reward konsisten. Bila sinyal berlawanan muncul saat posisi aktif, strategi akan melakukan flip langsung sehingga konteks `position`/`stop_level` ikut diperbarui pada frame sinyal.
...
- **Quick sanity check** – Notebook `notebooks/vwap.ipynb` dengan konfigurasi default + take profit 2.0×ATR menunjukkan Sharpe 2.05, CAGR 5.86%, dan max drawdown -0.52% pada fixture `mini_ohlcv`, membantu memvalidasi sesi terbatas/volatilitas rendah.
...
- **Varian long-only** – Jika ingin disiplin long-only dengan target profit, gunakan notebook `notebooks/ema_112.ipynb`: ATR stop 1.0× dan take profit 2.0× menghasilkan Sharpe 2.06, CAGR 6.56%, serta max drawdown -0.52% pada fixture `mini_ohlcv`.
...
- **`notebooks/indikator-ml-strategy/indicator_ml_strategy.ipynb`** – Fokus pada data indikator Binance BTCUSDT 1H/4H/1D dengan LightGBM + SHAP. Bagian akhir kini menyertakan *paket eksekusi 10 strategi* yang langsung menghitung posisi + metrik Sharpe/WinRate pada set uji (reversal, trend-following, breakout, ts-momentum, dsb.), lengkap dengan ensemble multi-timeframe 1H/4H/1D dan status untuk strategi yang membutuhkan data eksternal (funding/orderbook/multi-asset).

## Ringkasan indikator → ML → strategi → backtest (`indicator_ml_strategy.ipynb`)

- **Alur data & fitur**: notebook ini merapikan kolom indikator TradingView menjadi nama konsisten (termasuk komponen Ichimoku `ichi_conv`/`ichi_base`/`ichi_lag`/`ichi_span_a`/`ichi_span_b`), membuang kolom visual yang tidak dipakai, lalu membuat label `future_return` + `y` (klasifikasi naik/turun) dengan horizon 4 bar sebelum split time-based dan pemodelan LightGBM beserta pencarian threshold Sharpe di set validasi.【F:notebooks/indikator-ml-strategy/indicator_ml_strategy.ipynb†L188-L286】【F:notebooks/indikator-ml-strategy/indicator_ml_strategy.ipynb†L356-L575】
- **SHAP & kamus indikator Ichimoku**: SHAP rank menunjukkan `ichi_lag`, `ichi_conv`, dan `ichi_span_a/b` sering masuk jajaran fitur teratas lintas timeframe, menandakan konteks cloud/lagging span banyak dipakai model; tabel pentingnya ada di folder `outputs/shap/`. Kolom `ichi_*` masing-masing berasal dari kolom Ichimoku asli (Conversion/Base/Lagging/Leading Span A/B) sehingga pembaca dapat langsung menghubungkan nama variabel dengan panel indikatornya.【F:notebooks/indikator-ml-strategy/outputs/shap/shap_rank_table.csv†L1-L15】【F:notebooks/indikator-ml-strategy/indicator_ml_strategy.ipynb†L188-L209】
- **Strategi sesuai template**: paket 10 strategi di sel "Strategy pack implementation" memetakan `prob_up` hasil model ke posisi backtest-ready, plus panduan exit agar eksekusi lebih jelas:
  - *Reversal* (sideways): ADX < 20 atau band sempit; entry long jika `prob_up > 0.6` + RSI < 30 + %B < 0.1, short jika `prob_up < 0.4` + RSI > 70 + %B > 0.9. Exit: tutup saat RSI kembali ke 50–55 atau %B menembus 0.5; opsi hard stop jika ADX naik > 25 menandakan tren baru.
  - *Trend-following*: ADX > 25 dengan EMA cepat > lambat untuk bull (prob_up > 0.55) atau sebaliknya untuk bear (prob_up < 0.45). Exit: trailing stop ATR (mis. 2–3× ATR) atau potong posisi jika EMA cepat memotong balik EMA lambat / ADX turun < 20.
  - *Volatility mean reversion*: entry long saat %B < 0.05, RSI < 25, lebar band di atas median rolling, dan `prob_up > 0.65`. Exit: take-profit adaptif ketika RSI > 50 atau %B > 0.5; maksimum tahan 5–10 bar agar tidak tertahan di trend lanjut.
  - *Orderbook momentum*: memakai proxy `vol_rel_50` jika imbalance asli belum tersedia; long jika volume spike & `prob_up > 0.7`, short jika `prob_up < 0.3`. Exit: TP/SL ketat berbasis 1–2× ATR karena horizon scalping.
  - *Multi-timeframe ensemble*: posisi = 1 jika prob 1H/4H/1D masing-masing > 0.55/0.60/0.50; exit otomatis ketika salah satu timeframe gagal lolos ambang.
  - *Breakout & vol expansion*: donchian high + lonjakan volume, entry long jika `prob_up > 0.6` & BB width di atas ambang; exit jika harga kembali ke dalam range (close < donchian_high) atau volume/volatilitas mereda di bawah median rolling.
  - *TS momentum + vol targeting*: arah long bila return 20-bar di kuantil atas & `prob_up > 0.55`, short jika kuantil bawah & `prob_up < 0.45`; leverage disarankan diskalakan ke target volatil 2%–3% harian. Exit ketika momentum kuantil turun ke median atau leverage cap tercapai.
  - *Funding carry*: placeholder menunggu data funding/basis; rekomendasi exit: tutup saat funding kembali ke rata-rata 30D atau saat trend teknikal berlawanan (EMA cepat memotong lambat).
  - *Cross-sectional momentum*: placeholder multi-asset; exit ketika peringkat momentum keluar dari top-N / bottom-N atau saat likuiditas jatuh di bawah ambang volume.
  - *Stat-arb pairs*: placeholder; exit ketika z-score kembali ke -0.5 hingga 0.5 atau setelah max holding (mis. 3–5 hari) untuk menghindari drift spread.【F:notebooks/indikator-ml-strategy/indicator_ml_strategy.ipynb†L992-L1058】
- **Ensemble multi-timeframe & hasil tersimpan**: sel berikutnya menggabungkan prob 1H/4H/1D via `merge_asof` dan menerapkan aturan >0.55/0.60/0.50; output contoh menunjukkan Sharpe negatif (-0.66) pada sampel uji sehingga threshold/fitur perlu dituning ulang sebelum dipakai live.【F:notebooks/indikator-ml-strategy/indicator_ml_strategy.ipynb†L1077-L1130】
- **Kesesuaian pasar**: aturan di atas sudah mengikat pada rezim masing-masing—reversal menargetkan sideways/low ADX, trend-following untuk bull/bear kuat, mean-reversion untuk kondisi oversold ber-vol tinggi, breakout untuk fase ekspansi volatilitas, sementara strategi berbasis funding/orderbook/cross-asset disiapkan sebagai kerangka menunggu ketersediaan data. Semua blok mengikuti urutan Indikator → ML `prob_up` → mapping strategi → backtest metrik sehingga mudah diuji ulang per timeframe.
